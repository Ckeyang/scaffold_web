<template>
  <section class="w-full h-screen flex justify-center items-center bg-amber-200">
    <section class="border rounded w-1/3  min-w-[400px] max-w-[500px] px-4 py-8 bg-white">
      <el-input :placeholder="$t('login.account')" size="large" v-model="form.userAccount"></el-input>
      <el-input :placeholder="$t('login.password')" size="large" v-model="form.password" type="password"
                class="mt-8"></el-input>
      <el-input :placeholder="$t('login.code')" size="large" v-model="form.captchCode" class="mt-8">
