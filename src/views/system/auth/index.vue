<template>
  <section class="flex flex-col h-full">
    ...
    <section class="mt-8">
      <el-tooltip :content="$t('common.menu')" placement="bottom-start">
        <el-icon size="20">
          <Menu />
        </el-icon>
      </el-tooltip>
      <el-tooltip :content="$t('common.page')" placement="bottom-start">
        <el-icon size="20" class="mx-8">
          <Memo />
        </el-icon>
      </el-tooltip>

      <el-tooltip :content="$t('common.button')" placement="bottom-start">
        <el-icon size="20">
          <CirclePlus />
        </el-icon>
      </el-tooltip>
      ...
      <el-tree :data="dataList" :props="defaultProps">
        <template #default="{ node, data }">
          <section class="flex items-center">
            <el-icon v-if="data.type === 'GROUP'" size="20">
              <Menu />
            </el-icon>
            <el-icon v-if="data.type === 'INNER_LINK'" size="20">
              <Memo />
            </el-icon>
            <el-icon v-if="data.type === 'RESOURCE_INNER_LINK'" size="20">
              <CirclePlus />
            </el-icon>
...
import {
  ...
  getAuthTree,
  updateAuth,
} from "@/plugins/api/api-auth-controller";
...
const doAddAuth = async () => {
  const res: Response = await addAuth(form.value).then();
  if (res?.data?.code === 200) {
    getDataList();
  }
};
const doUpdateAuth = async () => {
  const res: Response = await updateAuth(form.value).then();
  if (res?.data?.code === 200) {
    getDataList();
  }
};
const doDelete = async (ids: number[]) => {
  const res: Response = await delAuthByIds(ids).then();
  if (res?.data?.code === 200) {
    getDataList();
  }
};
