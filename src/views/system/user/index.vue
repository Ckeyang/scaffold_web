        ...
        >
          <template #default="scope">
            <section class="flex justify-end">
              <el-button type="default" size="small" @click="addRole(scope.row)"
                >修改角色
              </el-button>
              <el-button
                type="primary"
                size="small"
                @click="doExportUser(scope.row)"
                >导出用户
              </el-button>
...
import {
  ...
  userSave,
  userUpdate,
} from "@/plugins/api/api-user-controller";
...
const doExportTemplate = async () => {
  const res = await exportTemplete();
  const url = await blobToUrl(res.data);
...
const doExportUser = async (item: UserForm) => {
  const res = await exportUser(item);
  const url = await blobToUrl(res.data);
  window.open(String(url));
};
/**
...
const deleteItem = async (item: UserForm) => {
  const res = await delUsers([item.id]);
  if (res.data.code === 200) {
    getDataList(true);
  }
};
...
const doUpdateRole = async () => {
  ...
  const res = await addUserRoles(role);
  if (res.data.code === 200) {
    cancel();
  }
};
